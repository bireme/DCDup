...
object Statistics extends App {
  ...
  private def stat(fname: String,
    ...
    val (totLines, colNumber, presenceCols, linePatterns) =
      stat(lines, totLines=0, colNumber=0, Map[Int, Int](), TreeMap[String, (Int, Int)](), separator)
    ...
    for (col <- 1 to colNumber) {
      val presence: Int = presenceCols.getOrElse(col, 0)
      val percent: Float = presence.toFloat/totLines*100
    ...
    for (used <- 0 to colNumber) {
      ...
      val percent: Float = tot.toFloat / totLines * 100
      ...
      sorted.toSeq.reverse.foreach {
        elem =>
          val percent2: Float =  elem._1.toFloat / tot * 100
  ...
  private def stat(lines: Iterator[String],
    ...
    if (lines.hasNext) {
      ...
      if (line.nonEmpty) {
        ...
        val colNum: Int = Math.max(colNumber, slen)

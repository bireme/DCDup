...
java -cp jar/DCDup-assembly-0.1.0.jar org.bireme.dcdup.CheckPipeFile "$1" "$2" "$3" "$4" "$5" "$6" -good="good_$NOW.txt" -bad="bad_$NOW.txt"
...
java -cp jar/DCDup-assembly-0.1.0.jar org.bireme.dcdup.Pipe2Lucene "$1" "$2" "$3" "$4" "$5" "$6" -pipe="good_$NOW.txt" -pipeEncoding=utf-8

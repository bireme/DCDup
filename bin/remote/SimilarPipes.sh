...
if [ "$#" -lt "5" ]
  then
    ...
    echo '   -dedupUrl=<DeDupBaseUrl> - DeDup url service  (http://dedup.bireme.org/services)'
...
java -cp target/scala-2.13/DCDup-assembly-0.1.0.jar org.bireme.dcdup.SimilarPipes "$1" "$2" "$3" "$4" "$5" "$6" "$7"

...
if [ "$#" -lt "8" ]
  then
    ...
    echo 'file) from a piped file and check then with itself and with a remote DeDup index'
    ...
    echo '[-pipeEncoding=<pipeFileEncoding>] - pipe file character encoding. Default is utf-8'
    exit 1
fi

cd /home/javaapps/sbt-projects/DCDup || exit

java -cp target/scala-2.13/DCDup-assembly-0.1.0.jar org.bireme.dcdup.WebDoubleCheckDuplicated $1 $2 $3 $4 $5 $6 $7 $8 $9
#sbt "runMain org.bireme.dcdup.WebDoubleCheckDuplicated $1 $2 $3 $4 $5 $6 $7 $8 $9"
